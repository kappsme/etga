...
def klogin(mysql, aplication_id):
    if (
        ...
        cursor.execute(
            "SELECT a.id, nivel, estado, bloqueo, k.state kapp_estado FROM kapps_db.accounts a left join kapps_db.kapps k on k.id=a.kapp_id WHERE username = %s and estado=1 \
