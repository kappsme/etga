...
app_etga.config["PDF_FOLDER"] = "templates/pdfs/"
app_etga.config["JSON_SORT_KEYS"] = False

# CONEXION A BASE DE DATOS
app_etga.config["MYSQL_HOST"] = os.getenv('MYSQL_HOST')
app_etga.config["MYSQL_CURSORCLASS"] = ""
app_etga.config["MYSQL_DB"] = os.getenv('MYSQL_DB')
app_etga.config["MYSQL_USER"] = os.getenv('MYSQL_USER')
app_etga.config["MYSQL_PASSWORD"] = os.getenv('MYSQL_PASSWORD')
mysql = MySQL(app_etga)



# Change this to your secret key (can be anything, it's for extra protection)
app_etga.secret_key = os.getenv('APP_SECRET_KEY')

# CONFIGURACION CORREO ELECTRONICO

app_etga.config["MAIL_SERVER"] = os.getenv('MAIL_SERVER')
app_etga.config["MAIL_PORT"] = os.getenv('MAIL_PORT')
app_etga.config["MAIL_USERNAME"] = os.getenv('MAIL_USERNAME')
app_etga.config["MAIL_PASSWORD"] = os.getenv('MAIL_PASSWORD')
...
app_etga.config["MAIL_USE_TLS"] = False
app_etga.config["MAIL_USE_SSL"] = False
mail = Mail(app_etga)
...
@app_etga.route("/login", methods=["GET", "POST"])
@app_etga.route("/", methods=["GET", "POST"])
def login():
    return klogin.klogin(mysql, aplication_id)


@app_etga.route("/home/", methods=["GET", "POST"])
def home():
    if "token" in session:
        ...
        if app_etga.config["MYSQL_DB"] == "tx_pruebas":
...
@app_etga.route("/nueva", methods=["POST"])
def nueva():
...
@app_etga.route("/busqueda_cliente", methods=["POST"])
...
@app_etga.route("/busqueda_cedula", methods=["POST"])
def busqueda_cedula():
    parametro = request.form.get("parametro")
    cursor = mysql.connection.cursor()
...
@app_etga.route("/crud_cliente", methods=["POST"])
...
@app_etga.route("/boleta_ingreso", methods=["POST"])
def boleta_ingreso():
...
@app_etga.route("/crud_catalogo", methods=["POST"])
def crud_catalogo():
...
@app_etga.route("/ingreso_catalogo", methods=["POST"])
...
@app_etga.route("/boleta1", methods=["POST"])
def boleta1():
...
@app_etga.route("/boleta2", methods=["POST"])
def boleta2():
...
# CRUD MOVIMIENTOS
@app_etga.route("/proceso_movimientos", methods=["POST"])
...
@app_etga.route("/cambio_estado", methods=["POST"])
def cambio_estado():
...
@app_etga.route("/set_impresiones", methods=["POST"])
def set_impresiones():
...
@app_etga.route("/impresion_boleta/<string:id_boleta>/<int:tipo>")
...
@app_etga.route("/pdfs_mail", methods=["POST"])
def pdfs_mail():
    tipo_accion = int(request.form["tipo_accion"])
    id_boleta = int(request.form["id_boleta"])
...
def envio_correo(tipo_mensaje, destinatario, adjunto=0, extra=0):
    if tipo_mensaje == 1:  # ENVIO DE RECIBO
        ...
        with app_etga.open_resource(adjunto) as fp:
...
@app_etga.route("/busqueda", methods=["POST"])
def busqueda():
    tipo_busqueda = int(request.form.get("tipo_busqueda"))
    cursor = mysql.connection.cursor()
...
@app_etga.route("/reportes", methods=["POST"])
def reportes():
...
@app_etga.route("/historial", methods=["POST"])
def historial():
...
@app_etga.route("/comentario", methods=["POST"])
def comentario():
    USUARIO = session["id"]
    id_boleta = request.form.get("id_boleta")
    ...
    if "token" in session:
        ...
        if request.endpoint != "logout" and request.endpoint != "login":
            ...
            cursor.execute(
                ...
                from kapps_db.accounts_log log left join kapps_db.accounts ac on ac.id=log.id_account inner join kapps_db.kapps k on k.id=ac.kapp_id and k.state='ACTIVE' \
...
@app_etga.route("/crud_usuario", methods=["POST"])
def crud_usuario():
...
@app_etga.route("/usuarios", methods=["POST"])
def usuarios():
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('SET lc_time_names = "es_ES"')
...
@app_etga.route("/inv", methods=["POST"])
def inv():
...
# TIENDA
@app_etga.route("/tnd", methods=["POST"])
def tnd():
...
@app_etga.route("/imagenes/<codigo>/<imagen>", methods=["GET"])
def imagenes(codigo, imagen):
    with open(
        os.path.join(
            app_etga.root_path, "inv_imagenes", codigo + "_" + imagen + ".jpg"
        ),
        "rb",
    ) as f:
        image_binary = f.read()
...
# INVENTARIO IMAGENES (desuso)
@app_etga.route("/imagenes_inventario2", methods=["POST"])
...
# TIENDA
@app_etga.route("/imagenes_inventario", methods=["POST"])
...
if __name__ == "__main__":
    app_etga.run(host="0.0.0.0", port=os.getenv('SERVER_PORT'), debug=True)
